...
namespace Nop.Plugin.Misc.FreshAddressNewsletterIntegration.Controllers
{
    ...
    public class CustomNewsletterController : BasePublicController
    {
        ...
        [HttpPost]
        [IgnoreAntiforgeryToken]
        public virtual async Task<IActionResult> SubscribeSMS(string email, string smsOptIn, string marketingSmsOptIn, bool subscribe)
        {
            string result;
            var success = false;
            ...
            else
            {
                ...
                else if (subscribe)
                {
                    ...
                    result = await _localizationService.GetResourceAsync("Newsletter.SubscribeSMSSent");
                }
                else
                {
                    result = await _localizationService.GetResourceAsync("Newsletter.UnsubscribeEmailSent");
